...
function Step1({ onClose, onSend }: Step1Props) {
  ...
  const onInvalid: SubmitErrorHandler<Step1Form> = (errors) => {
    console.error({ form: 'gift-deposit-step-1', errors });
  };
...
function Step2({ onClose, onSend }: Step2Props) {
  ...
  const onInvalid: SubmitErrorHandler<Step2Form> = (errors) => {
    console.error({ form: 'gift-deposit-step-2', errors });
  };
...
function Step3({ onConfirm }: Step3Props) {
  return (
    <YStack>
      <GiftSummary />
      <Button grow={1} variant="primary" onPress={onConfirm}>
        Confirm
      </Button>
    </YStack>
  );
...
export function GiftDepositSheet({ open, setOpen }: Props) {
    ...
    >
      ...
      {step === 3 && (
        <Step3
          onConfirm={() => {
            router.navigate({
              pathname: '/(dashboard)/gift-success',
