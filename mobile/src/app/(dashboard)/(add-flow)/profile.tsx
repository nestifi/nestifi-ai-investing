    ...
    <BaseLayout minH="100%">
          ...
          <YStack rounded={16} p={16} gap={12} bg={COLORS.additional.blue}>
            <XStack gap={12} items="center" flexWrap="nowrap">
              ...
              <View justify="center" ml="auto">
                {child.futureGoals.length > 0 ? (
                  <PencilIcon color={COLORS.grey[100]} />
                ) : (
                  <AddIcon color={COLORS.grey[100]} />
                )}
              </View>
            </XStack>
            {child.futureGoals.length > 0 && (
                ...
                {child.familyCircle.map((el, idx) => (
                  <YStack key={idx} gap={4} justify="center" items="center" width={`${100 / 4 - 2}%`}>
                    ...
                    <Avatar circular size="$6">
                      <Avatar.Image src={adultAvatar1ImageUri} />
      ...
      {investment && (
        <Sheet
          ...
          title="Confirmation"
          desc={`You are investing $${investment.amount} ${investment.frequency.type === 'weekly' ? 'per week' : 'per month'} into a ${investment.product} on behalf of ${child.firstName} ${child.lastName}`}
        >
          <XStack gap={8}>
            <Button grow={1} variant="secondary" fontWeight={500} onPress={() => setConfirmationOpen(false)}>
              Back
            </Button>
            <Button
              grow={1}
